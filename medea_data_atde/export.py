...
def date2set_index(df, year, tz='utc', set_sym='h'):
    ...
    if len(df) == hours_in_year(year):
        ...
        raise ValueError(
            'Mismatch of time series data and model time resolution. Is year wrong?')
    return df
...
def compile_symbols(root_dir, timeseries, zones, weatheryear, year,
    ...
    technologies = {
        'capacity': pd.read_csv(package_dir / 'raw' / 'capacities.csv',
                                index_col=[0, 1, 2, 3]),
        'capacity_transmission': pd.read_csv(package_dir / 'raw' / 'transmission.csv',
                                             index_col=[0, 1]),
        'technology': pd.read_csv(package_dir / 'raw' / 'technologies.csv',
                                  index_col=[0]).dropna(axis=0, how='all'),
        'operating_region': pd.read_csv(package_dir / 'raw' / 'operating_region.csv',
                                        index_col=[0, 1, 2]),
    }
    ...
    estimates = {
        'external_cost': pd.read_csv(package_dir / 'raw' / 'external_cost.csv',
                                     index_col=[0]),
        'point_estimates': pd.read_csv(package_dir / 'raw' / 'point_estimates.csv',
                                       index_col=[0]),
        'price_nonmarket_fuels': pd.read_csv(
            package_dir / 'raw' / 'price_nonmarket_fuels.csv', index_col=[0]),
    }

    # create SETS
    # --------------------------------------------------------------------------- #
    sets = {
        'e': [carrier for carrier in
              np.unique(technologies['technology'][['fuel', 'primary_product']])],
        # all energy carriers
        'i': [input for input in technologies['technology']['fuel'].unique()],
        # energy inputs
        'f': [final for final in
              technologies['technology']['primary_product'].unique()],  # final energy
        't': [tec for tec in technologies['technology'].index.unique()],  # technologies
        'c': [chp for chp in
              technologies['operating_region'].index.get_level_values(0).unique()],
        # co-generation technologies
        'd': [plant for plant in technologies['technology'].loc[
            technologies['technology']['conventional'] == 1].index.unique()],
        # dispatchable technologies
        'r': [intmit for intmit in technologies['technology'].loc[
            technologies['technology']['intermittent'] == 1].index.unique()],
        # intermittent technologies
        's': [storage for storage in technologies['technology'].loc[
            technologies['technology']['storage'] == 1].index.unique()],
        # storage technologies
        'g': [transmit for transmit in technologies['technology'].loc[
            technologies['technology']['transmission'] == 1].index.unique()],
        # transmission technologies
        'l': [f'l{x}' for x in range(1, 5)],  # feasible operating regions
        'h': [f'h{hour}' for hour in range(1, hours_in_year(weatheryear) + 1)],
        # time steps / hours
    ...
    technologies['operating_region']['fuel_need'] = technologies['operating_region'][
                                                        'fuel'] / \
    ...
    technologies['distance'] = technologies['capacity_transmission']['distance'].loc[
                               technologies['capacity_transmission'][
                                   'distance'].index.get_level_values(0).str.contains(
                                   '|'.join(zones)) &
                               technologies['capacity_transmission'][
                                   'distance'].index.get_level_values(1).str.contains(
                                   '|'.join(zones)), :]
    # transmission capacities
    technologies['capacity_transmission'] = technologies[
        'capacity_transmission'].assign(f='el').set_index('f',
                                                          append=True)
    technologies.update(
        {'capacity_transmission': technologies['capacity_transmission']['ATC'].loc[
                                  technologies['capacity_transmission'][
                                      'ATC'].index.get_level_values(
    ...
    ts_data['zonal'] = ts_data['timeseries'].loc[:,
                       ts_data['timeseries'].columns.str.startswith(
                           tuple(zones))].copy()
    ...
    ts_data['timeseries'].loc[:, 'Nuclear'] = \
        estimates['price_nonmarket_fuels'].loc['Nuclear', :].values[0]
    ts_data['timeseries'].loc[:, 'Lignite'] = \
        estimates['price_nonmarket_fuels'].loc['Lignite', :].values[0]
    ts_data['timeseries'].loc[:, 'Biomass'] = \
        estimates['price_nonmarket_fuels'].loc['Biomass', :].values[0]

    model_prices = ['Coal', 'Oil', 'Gas', 'EUA', 'Nuclear', 'Lignite', 'Biomass',
                    'price-day_ahead']
    ...
    hydro_storage = technologies['technology'].loc[
        (technologies['technology']['storage'] == 1) &
        (technologies['technology'].index.str.contains('hyd'))].index
    inflow_factor = technologies['capacity'].unstack(1).loc[
                        idx['Installed Capacity Out',
                            year, hydro_storage], idx[:, zones]] / \
                    technologies['capacity'].unstack(1).loc[
                        idx['Installed Capacity Out',
                            year, hydro_storage], idx[:, zones]].sum()
    inflow_factor.columns = inflow_factor.columns.droplevel([0])
    inflow_factor.index = inflow_factor.index.droplevel([0, 1])
    ...
    for zone in list(zones):
        for strg in hydro_storage:
            ts_inflows.loc[:, (zone, strg)] = ts_data['zonal'].loc[:,
                                              idx[zone, 'reservoir', 'inflows']] * \
    ...
    # --------------------------------------------------------------------------- #
    ts_data.update({'PEAK_LOAD': ts_data['zonal'].loc[:,
                                 idx[:, 'el', 'load']].max().unstack((1, 2)).squeeze()})
    peak_profile = ts_data['zonal'].loc[:, idx[:, :, 'profile']].max().unstack(2).drop(
        'Water', axis=0, level=1)
    ...
    map_input = pd.DataFrame(data=False, index=technologies['technology'].index,
                             columns=np.unique(technologies['technology'][
                                                   ['fuel', 'primary_product']]))
    ...
    map_output = pd.DataFrame(data=False, index=technologies['technology'].index,
                              columns=technologies['technology'][
                                  'primary_product'].unique())
    for ix, row in technologies['technology'].iterrows():
        map_output.loc[ix, row['primary_product']] = True
    map_output.loc[map_output.index.str.contains('chp'), 'ht'] = True

    storage_fuels = map_input.apply(lambda x: ','.join(x.index[x == True]), axis=1)
    # CAPACITY_STORAGE
    cap_store = technologies['capacity'].loc[idx['Storage Capacity', zones, year,
                                                 [s[0] for s in sets['s'].values]], :]
    cap_store.index = cap_store.index.droplevel((0, 2))
    cap_store = cap_store.assign(f=[storage_fuels.loc[tec] for tec in cap_store.index.get_level_values(1)])
    cap_store = cap_store.set_index('f', append=True).reorder_levels((0, 2, 1))

    # CAPACITY_STORAGE_IN
    cap_store_in = technologies['capacity'].loc[
                   idx['Installed Capacity In', zones, year,
                       [s[0] for s in sets['s'].values]], :]
    cap_store_in.index = cap_store_in.index.droplevel((0, 2))
    cap_store_in = cap_store_in.assign(f=[storage_fuels.loc[tec] for tec in cap_store_in.index.get_level_values(1)])
    cap_store_in = cap_store_in.set_index('f', append=True).reorder_levels((0, 2, 1))

    # CAPACITY_STORAGE_OUT
    cap_store_out = technologies['capacity'].loc[
                   idx['Installed Capacity Out', zones, year,
                       [s[0] for s in sets['s'].values]], :]
    cap_store_out.index = cap_store_out.index.droplevel((0, 2))
    cap_store_out = cap_store_out.assign(
        f=[storage_fuels.loc[tec] for tec in cap_store_out.index.get_level_values(1)])
    cap_store_out = cap_store_out.set_index('f', append=True).reorder_levels((0, 2, 1))
    ...
    parameters = {
        'AIR_POL_COST_FIX': [['i'], estimates['external_cost']['fixed cost'].dropna(),
                             '[EUR per MW]'],
        'AIR_POL_COST_VAR': [['i'],
                             estimates['external_cost']['variable cost'].dropna(),
                             '[EUR per MWh]'],
        'CAPACITY': [['z', 't'], technologies['capacity'].loc[
                                 idx['Installed Capacity Out', zones, year], :].unstack(
            [0, 2]), '[GW]'],
        'CAPACITY_X': [['z', 'z', 'f'], technologies['capacity_transmission'], '[GW]'],
        'CAPACITY_STORAGE': [['z', 'f', 's'], cap_store, '[GWh]'],
        'CAPACITY_STORE_IN': [['z', 'f', 's'], cap_store_in, '[GW]'],
        'CAPACITY_STORE_OUT': [['z', 'f', 's'], cap_store_out, '[GW]'],
        'OVERNIGHTCOST': [['t'], technologies['technology'].loc[:, 'capex_p'].round(4),
                          '[EUR per MW]'],
        'OVERNIGHTCOST_E': [['s'], technologies['technology'].loc[
            [s[0] for s in sets['s'].values], 'capex_e'], '[EUR per MWh]'],
        'OVERNIGHTCOST_P': [['t'], technologies['technology'].loc[
            [s[0] for s in sets['s'].values], 'capex_p'], '[EUR per MW]'],
        'OVERNIGHTCOST_X': [['g'], technologies['technology'].loc[
            [g[0] for g in sets['g'].values], 'capex_p'], '[EUR per MW]'],
        'CO2_INTENSITY': [['i'], estimates['external_cost']['CO2_intensity'].dropna(),
                          '[t CO2 per MWh fuel input]'],
        'CONVERSION': [['e', 'f', 't'], technologies['technology'][
            ['fuel', 'primary_product', 'eta_ec']].reset_index().set_index(
            ['fuel', 'primary_product', 'set_id']), '[]'],
        ...
        'DEMAND': [['z', 'h', 'f'], ts_data['zonal'].loc[
                                    :, idx[:, :, 'load']].stack((0, 1)).reorder_levels(
            (1, 0, 2)).round(4), '[GW]'],
        'DISCOUNT_RATE': [['z'], estimates['point_estimates'].loc['wacc', :], '[]'],
        'DISTANCE': [['z', 'z'], technologies['distance'], '[km]'],
        'FEASIBLE_INPUT': [['l', 'i', 'c'],
                           technologies['operating_region']['fuel'].div(
                               technologies['technology']['eta_ec']).reorder_levels(
                               (1, 2, 0)), '[GW]'],
        ...
        'PRICE_CO2': [['z', 'h'],
                      ts_data['price'].loc[:, idx['EUA', :]].stack().reorder_levels(
                          (1, 0)), '[EUR per t]'],
        'PRICE': [['z', 'h', 'i'],
                  ts_data['price'].drop(columns=['EUA'], level=0).stack(
                      (0, 1)).reorder_levels((2, 0, 1)).round(4), '[EUR per MWh]'],
        'PRICE_TRADE': [['i'], estimates['price_nonmarket_fuels'].loc['Syngas', :],
                        '[EUR per MWh]'],
        'PROFILE': [['z', 'h', 'i'],
                    ts_data['zonal'].loc[:, idx[:, :, 'profile']].stack(
                        (0, 1)).reorder_levels((1, 0, 2)).round(4), '[]'],
        'SIGMA': [['z'], estimates['point_estimates'].loc['SIGMA', :], '[]'],
        'VALUE_NSE': [['z'], estimates['point_estimates'].loc['VALUE_NSE', :],
                      '[EUR per MWh]'],
